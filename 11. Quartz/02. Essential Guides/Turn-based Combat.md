...
**This is just a short TL;DR section to show you where to check *Base SPD*.**
Characters' Base Stats, including Base SPD (which we will be mentioning here), are found by clicking the "More Stats" button when checking a character's details. **The Base Stats are in gray.** 
...
2. Calculate the AV taken for March 7th's first turn. This Action Value passes not only for March 7th, but for Feixiao too.
$$150={7500\over 127}$$
1. Calculate the AV taken for one of Feixiao's turns without the speed buff.
$$150={7500\over 127}+{10000\over x}$$
1. Subtract from Step 3's part of the calculation, the AV taken for March 7th's first turn at the start of battle with her 25% Action Forward.
$$150={7500\over 127}+{10000\over x}-{7500\over127}$$This step, together with the previous step, calculates the amount of Action Value Feixiao still needs to take until she can get to her first turn.
...
1. Multiply the result of the subtraction between step 2 and 3 by Feixiao's SPD.
$$150={7500\over 127}+({10000\over x}-{7500\over127})\times x$$
1. Divide the result of Step 5 by Feixiao's new SPD, after it gets buffed by March 7th.
$$150={7500\over 127}+{({10000\over x}-{7500\over127})\times x\space÷\space(x+11.2)}$$
1. ***Final Step*** Finally, after finding out the total AV taken by Feixiao before and after March's SPD buff, all we need to do is to calculate Feixiao's 2nd turn AV and add it in.
...
Considering the above, the steps you will want to take to find the answer are as follows:
2. We want to find Firefly's Required SPD for acting **within the first cycle after changing waves**.
...
3. Calculate for all 3 of Firefly's Turns' AV as if they were all performed with her Ultimate's SPD Buff active. x = Firefly's SPD outside of battle.
$$150={3\times 10000\over x+70.4}$$
1. Subtract from the result, the amount of AV Firefly's Ultimate lasts for.
$$150={30000\over x+70.4}-{10000\over70}$$We do this to find the Remaining AV that still needs to be travelled after the Ultimate ends.
...
1. Take the result from step 2 and multiply it by the SPD Firefly has during her Ultimate.
$$150=({30000\over x+70.4}-{10000\over70})\times (x+70.4)$$By doing this, we turn the left-over Action Value into Action Gauge.
...
1. Take the result from step 3 and divide it by the SPD Firefly has out-side of her Ultimate.
$$150={({30000\over x+70.4}-{10000\over70})\times (x+70.4)\over x+10.4}$$As said in the previous step's explanation, we are performing this to find Firefly's AV taken for the 4th turn out-side of her ultimate **according to Firefly's "x" SPD** that we ultimately want to find.
After this, the only thing left to do is add to the result the amount of AV Firefly's Ultimate lasts for.

1. ***Final Step*** Take the result from step 5 and add the AV duration of Firefly's Ultimate.
$$150={({30000\over x+70.4}-{10000\over70})\times (x+70.4)\over x+10.4}+{10000\over 70}$$This is the final version of the calculation. If you've understood the steps thus far then you understand how the calculation works and how we've come to it. 
...
Credit to Mr. Pokke for this image and graph.
...
2. Using Dance! Dance! Dance! light cone
- S5 DDD = 184.000
- S4 DDD = 185.334
- S3 DDD = 186.667
- S2 DDD = 188.000
...
3. Using ONLY 4-Pc Eagle of Twilight Line set effect = 183.334

4. Using DDD and 4-Pc Eagle of Twilight Line
...
##### **Effective 160 SPD (4 Turns in first 2 cycles)**
- 4-Pc Eagle = 150.000
- S5 DDD = 150.400
- S4 DDD = 151.200
- S3 DDD = 152.000
